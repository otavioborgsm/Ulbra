...
insert into pro_mov_transferencias(quant_produto,id_produto,id_tranferencia)
	  values(10,3,3);
...
begin
	...
	set centroDeCustoSaida = (select id_centro_custo from saidas where id = new.id_saida);
    
	update pro_centros_custos set quant_produto = quant_produto - new.quant_produto
	where  id_centro_custo = centroDeCustoSaida and id_produto = new.id_produto;
...
-- Atualiza estoque do centro de custo quando acontece uma transferência

DELIMITER $$
	CREATE TRIGGER atualiza_estoque_transferencia
		after insert on  pro_mov_transferencias
			for each row
begin

	DECLARE centroDeCustoSaida int;
    DECLARE centroDeCustoDestino int;
    DECLARE produtoCentroDeCusto int;
	set centroDeCustoSaida = (select id_centro_custo from transferencias where id = new.id_tranferencia);
    set centroDeCustoDestino = (select id_centro_custo_destino from transferencias where id = new.id_tranferencia);
	set produtoCentroDeCusto = (select id_produto from pro_centros_custos where new.id_produto = id_produto and id_centro_custo = centroDeCustoDestino  LIMIT 1 );
    
    update pro_centros_custos set quant_produto = quant_produto - new.quant_produto
	where  id_centro_custo = centroDeCustoSaida and id_produto = new.id_produto;
    
    if (new.id_produto = produtoCentroDeCusto ) then
		update pro_centros_custos set quant_produto = quant_produto + new.quant_produto
		where  id_centro_custo = centroDeCustoDestino and id_produto = new.id_produto;
    else 
		insert into pro_centros_custos(quant_produto, id_produto, id_centro_custo)
		values(new.quant_produto,  new.id_produto, centroDeCustoDestino);
    end if;
...
insert into transferencias(data_movimento,id_centro_custo,id_centro_custo_destino,id_almoxarife,id_almoxarife_destino)
	values('2021-04-8',2,1,3,1);

insert into pro_mov_transferencias(quant_produto,id_produto,id_tranferencia)
	values (1,4,5);
    

-- Atualiza tabela pro_centros_custos quando da entrada de uma nota fiscal

DELIMITER $$
	CREATE TRIGGER atualiza_estoque
		after insert on  pro_notas
			for each row
begin

    DECLARE centroDeCustoDestino int;
    DECLARE produtoCentroDeCusto int;
    set centroDeCustoDestino = (select id_centro_custo from nf_entradas where id = new.id_nf_entrada);
	set produtoCentroDeCusto = (select id_produto from pro_centros_custos where new.id_produto = id_produto and id_centro_custo = centroDeCustoDestino  LIMIT 1 );
